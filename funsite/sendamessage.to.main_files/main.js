    ...
    NLField.prototype = {
        ...
        _createDropDown : function() {
            ...
            Array.prototype.slice.call( this.elOriginal.querySelectorAll( 'option' ) ).forEach( function( el, i ) {
                if(el.innerHTML !== 'your message') {
                    ihtml += self.elOriginal.selectedIndex === i ? '<li class="nl-dd-checked" data-background="'+el.dataset.background+'">' + el.innerHTML + '</li>' : '<li data-background="'+el.dataset.background+'">' + el.innerHTML + '</li>';
            ...
            this.elOriginal.removeChild(this.elOriginal[0]);
            this.selectedIdx = 1;
        },
        ...
        _open : function() {
            ...
            var $overlaySelectorList=$(".nl-dd ul");
            var $currentTextField=$(".nl-field-toggle");
            var $clickableOverlay=$(".nl-overlay");
            var $stageHeight=$(window).height();

            //calculate margins
            var beforeAfterMargin=$overlaySelectorList.width()-$currentTextField.width();

            //position overlay at center
            var offsetX=($currentTextField.width()-$overlaySelectorList.width())*.5;

            TweenMax.set($overlaySelectorList,{
            	x:offsetX,
            	scale:0,
            	autoAlpha:0
            });

            TweenMax.to($overlaySelectorList,.5,{
            	scale:1,
            	autoAlpha:1,
            	ease:Expo.easeOut
            });

            //position left and right elements margins
            TweenMax.to(".send-before",.6,{marginRight:beforeAfterMargin/2,ease:Expo.easeOut});
            TweenMax.to(".send-after",.6,{marginLeft:beforeAfterMargin/2,ease:Expo.easeOut});

			TweenMax.set($clickableOverlay, {
                height:$stageHeight,
                y:-$(".send-what").offset().top
            });

            TweenMax.to($clickableOverlay, 0.25, {
                autoAlpha: 0.6
            });
        ...
        close : function( opt, idx ) {
            ...
            //position left and right elements margins
            TweenMax.to(".send-before",.4,{marginRight:0,ease:Expo.easeOut});
            TweenMax.to(".send-after",.4,{marginLeft:0,ease:Expo.easeOut});

            //remove overlay selector
            TweenMax.set($overlaySelectorList,{
            	x:offsetX
            })

             TweenMax.to($overlaySelectorList,.6,{
            	scale:0,
            	autoAlpha:0,
            	ease:Expo.easeOut
            });
...
/* global TweenLite, TweenMax, TimelineMax, Expo, Linear, _, log, ZeroClipboard, requestAnimFrame, NLForm, ga, Howl */

var App = {
    name: {}, // either String or Bool, that's why instantiated as an Object
    totalFrames: 0,
    imageHeight: 0,
    imageDir: '',
    isSent: false,
    ...
    init: function() {
        // check for screen size and density, load different images depending on it
        if(window.innerHeight <= 700){
            App.imageDir = App.imageDir + '-700';
        }else if(window.devicePixelRatio <= 1){
            App.imageDir = App.imageDir + '-1080';
        }
        // setup the NLForm
        App.nlform = new NLForm(document.getElementById('nl-form'));
        // preloading the images
        App.preloadImages();
        // init scrollDownAnimation
        App.navigation.initScrollDownAnimation();
        // kick off the rAF
        App.animLoop();
        // size the dummy for the smooth scrolling
        $('.dummy').height(App.imageHeight);
        // start bindings
        App.registerEvents();
        // animate the intro
        App.navigation.to.startScreen();

        // load the sound
        App.sounds.whistle = new Howl({
          urls: ['/dist/audio/whistle.mp3','/dist/audio/whistle.ogg']
        });

        $('#theName').keyup(function() {
            App.helpers.shrinkToFill(this, App.inputFontSize, "800", "proxima-nova, sans-serif");
        });
        ...
        $(document).mousewheel(function(event) {
            ...
            var Yoffset = $('.wrapper').scrollTop();

            var progress = Math.round(Yoffset / (App.imageHeight - $(window).height())*100)/100;
            if(progress!==App.oldProgress){
                ga('send', 'event', 'intro', 'scrolled_down',progress);
            }
            App.oldProgress=progress;


            // App.scrollPosition-=event.deltaY * event.deltaFactor;
           // if(App.scrollPosition<0)App.scrollPosition=0;
           // if(event.deltaY>0&&App.scrollPosition>App.imageHeight)App.scrollPosition=App.imageHeight;
           // if(App.scrollPosition>offsetHeight)App.scrollPosition=offsetHeight;

           // App.animations.scrollDown.progress(progress);
        });


    },
    preloadImages: function(){
        // TODO: schlechte verbindung = kleinere Größe
        $('#video').css('backgroundImage', 'url(/dist/images/'+App.imageDir+'/1.jpg)');
        for(var i = 1; i < App.totalFrames + 1; i++) {
            App.images[i] = new Image();
            App.images[i].src = "/dist/images/"+App.imageDir+"/"+i+".jpg";
        }
    },
    animLoop: function(){
        var Yoffset = $('.wrapper').scrollTop();
        App.loop = window.requestAnimationFrame(App.animLoop);
        App.targetStep = Math.max( Math.round( Yoffset / 30 ) , 1 ); // what frame to animate to
        if(App.targetStep !== App.step ) { App.step += (App.targetStep - App.step) / 5; } // increment the step until we arrive at the target step
        App.changeFrame();
        // animate on progress change
        var progress = Yoffset / (App.imageHeight - $(window).height());
        App.animations.scrollDown.progress(progress);
    },
    changeFrame: function() {
        var thisStep = Math.round(App.step); // calculate the frame number
        if(App.images.length > 0 && App.images[thisStep]) { // if the image exists in the array
            if(App.images[thisStep].complete) { // if the image is downloaded and ready
                if($('#video').css('backgroundImage') !== 'url('+App.images[thisStep].src+')') { // save overhead...?
                    $('#video').css('backgroundImage', 'url('+App.images[thisStep].src+')');
                }
            }
        }
    },
    navigation: {
        state: false,
        initScrollDownAnimation: function(){
            App.animations.scrollDown = new TimelineMax({ paused: true});
            App.animations.scrollDown.to('.scroll-down', 5, {
                y: 50,
                autoAlpha: 0
            });
            App.animations.scrollDown.set('.message span', {
                scale: 0.9
            });
            App.animations.scrollDown.set('#sendYourOwnButton', {
                scale: 0.9,
                ease: Expo.easeOut
            });
            App.animations.scrollDown.set('.dummy', {
                pointerEvents: 'auto'
            });
            App.animations.scrollDown.to('.video-container', 1, {
                opacity: 0.8
            });
            App.animations.scrollDown.call(function(){
                if(App.soundPlayed.whistle === false && App.imageDir === 'hungergames'){
                    App.soundPlayed.whistle = true;
                    App.sounds.whistle.play();
                }
            });
            App.animations.scrollDown.staggerTo('.message span', 1, {
                autoAlpha: 1,
                scale: 1,
                ease: Expo.easeOut
            }, 0.5, '-=0.25');
            App.animations.scrollDown.to('#sendYourOwnButton', 1, {
                autoAlpha: 1,
                scale: 1,
                ease: Expo.easeIn
            }, '-=0.25');
            App.animations.scrollDown.to('.bottom-right', 0.3, {autoAlpha: 1});
            if(App.name === false){
                App.animations.scrollDown.to('.reload', 0.5, {
                    y: 0,
                    ease: Expo.easeOut
                }, '-=0.1');
            }
        ...
        to: {
            startScreen: function(){
                ...
                TweenMax.set('.madebywild', {
                    y: 20,
                    scale: 0.9,
                    autoAlpha: 0
                });
                // animate
                TweenLite.to('#video', 1.5, {
                    delay: 0.5,
                    y: 0,
                    ease: Expo.easeOut
                });
                TweenMax.to('.scroll-down', 1, {
                    delay: 1.8,
                    y: 0,
                    scale: 1,
                    alpha: 1
                });
                TweenMax.to('.madebywild', 1, {
                    delay: 2.5,
                    y: 0,
                    scale: 1,
                    autoAlpha: 0.4
                });
                App.animations.startScreen = new TimelineMax({repeat: -1});
                // App.animations.startScreen.timeScale(0.5);
                App.animations.startScreen.set($('.arrows').children(), {
                    y: -10
                })
                App.animations.startScreen.set('.arrows .first', {opacity: 0.1});
                App.animations.startScreen.set('.arrows .second', {opacity: 0.75});
                App.animations.startScreen.set('.arrows .third', {opacity: 0.5});
                App.animations.startScreen.to('.arrows .first', 1, {
                    y: 0,
                    opacity: 0.75,
                    ease: Linear.easeNone
                });
                App.animations.startScreen.to('.arrows .second', 1, {
                    y: 0,
                    opacity: 0.5,
                    ease: Linear.easeNone
                }, "-=1");
                App.animations.startScreen.to('.arrows .third', 1, {
                    y: 0,
                    opacity: 0.1,
                    ease: Linear.easeNone
                }, "-=1");
            ...
            sendWhat: function() {
                ...
                // kill raf
                window.cancelAnimationFrame(App.loop);
                App.animLoop = null;
                App.loop = null;
                TweenMax.to('.reload', 1, {
                    y: -200
                });

                 TweenMax.to('.scroll-down', 0.5, {
                    autoAlpha:0
                });

                TweenLite.to($('.video-container'), 0.2, {autoAlpha: 0});
                TweenLite.to($('.last-screen'), 0.3, {autoAlpha: 0, onComplete: function() {
                    TweenLite.set($('.last-screen'), {display: 'none'});
                }});
                TweenLite.fromTo($('.send-what'), 0.3, {y: 40},{y: 60, autoAlpha: 1, delay: 0.5});
            },
            sendTo: function() {
                TweenLite.to($('.send-what'), 0.5, {y: 0});
                TweenMax.fromTo($('.send-to'), 0.8, {y: 20},{y: 0, autoAlpha: 1, onComplete: function(){
                    $('#theName').focus();
                }});
            },

            createLink: function() {
                ga('send', 'pageview', '/sendtofriend');

                App.helpers.handleInput();
                App.animations.createLinkAnimation = new TimelineMax();
                // setup
                App.animations.createLinkAnimation.set('.url', {
                    y: -20,
                    scale: 0.95
                });
                App.animations.createLinkAnimation.set($('.send-screen .helpermessage').first(), {
                    y: -20,
                    scale: 0.95
                });
                App.animations.createLinkAnimation.set('.send-screen .social-share', {
                    y: -20,
                    scale: 0.95
                });
                App.animations.createLinkAnimation.set($('.send-screen .helpermessage').last(), {
                    y: -20,
                    scale: 0.95
                });
                TweenMax.set('.further-updates', {
                    y: 20,
                    scale: 0.9,
                    autoAlpha: 0
                });
                // animating
                App.animations.createLinkAnimation.to('.send-what', 0.3, {
                    y: -20,
                    autoAlpha: 0
                });
                App.animations.createLinkAnimation.to('.send-to', 0.2, {
                    y: -20,
                    autoAlpha: 0
                }, '-=0.1');
                App.animations.createLinkAnimation.set('.create-screen', {display: 'none'});
                App.animations.createLinkAnimation.to('#createLinkButton', 0.2, {y: -20, autoAlpha: 0});
                App.animations.createLinkAnimation.to('.url', 0.3, {
                    autoAlpha: 1,
                    scale: 1,
                    y: 0
                }, '+=0.3');
                App.animations.createLinkAnimation.to($('.send-screen .helpermessage').first(), 0.3, {
                    autoAlpha: 0.4,
                    scale: 1,
                    y: 0
                }, '-=0.2');
                App.animations.createLinkAnimation.to('.send-screen .social-share', 0.3, {
                    autoAlpha: 1,
                    scale: 1,
                    y: 0
                }, '-=0.2');
                App.animations.createLinkAnimation.to($('.send-screen .helpermessage').last(), 0.3, {
                    autoAlpha: 0.4,
                    scale: 1,
                    y: 0
                }, '-=0.2');
                App.animations.createLinkAnimation.to('.further-updates', 1, {
                    autoAlpha: 1,
                    scale: 1,
                    y: 0
                }, '-=0.2');
    ...
    helpers: {
        handleInput: function() {
            var name = $('#theName').val(),
                type = $('#nl-form select').val(),
                domain = "sendamessage.to/",
                url = (domain + name + '/' + type).toLowerCase();
                url = url.replace(new RegExp(" ", "g"),"+");
            ga('send', 'event', 'customize', 'selected',$('#nl-form select').val());


            $('#urlToSend').val(url);

            $(".facebook").attr( "href", "https://www.facebook.com/sharer/sharer.php?u=http://"+url );
            $(".twitter").attr( "href", "https://twitter.com/home?status=Just%20send%20a%20message!%20http://"+url );
            $(".google").attr( "href", "https://plus.google.com/share?url=http://"+url );
            $(".mail").attr( "href", "mailto:someone@you.know?subject=Check%20this%20message!&body=http://"+url );
        ...
        setBackgroundPicture: function(image) {
            var bgSet = new TimelineMax();
            bgSet.to('.video-container', 0.1, {autoAlpha: 0});
            bgSet.set('.video-container', {y: 40});
            bgSet.set('#video', {'background-image': 'url('+image+')'});

            bgSet.to('.video-container', 0.9, {y:0});
            bgSet.to('.video-container', 2, {autoAlpha:0.6},"-=0.9");
        ...
        map:function(num, min1, max1, min2, max2, round, constrainMin, constrainMax){
            round=round || false;
            constrainMin = constrainMin || true;
            constrainMax = constrainMax || true;
        ...
        shrinkToFill: function(input, fontSize, fontWeight, fontFamily) {
            var $input = $(input),
                txt = $input.val(),
                maxWidth = $input.width() + 5, // add some padding
                font = fontWeight + " " + fontSize + "px " + fontFamily;
            // see how big the text is at the default size
            var textWidth = App.helpers.measureText(txt, font).width;
    ...
    registerEvents: function() {
        // Logo Hover
        $('.madebywild').hover(function(){
            TweenMax.to('.madebywild', 0.5, {
                opacity: 1
            });
            TweenMax.to('.madebywild svg', 0.5, {
                ease: Expo.easeOut,
                scale: 1.25
            });
        },function(){
        ...
        // Scroll "back" on last-screen
        $('.last-screen').on('mousewheel', function(e){
            if(e.deltaY > 0){
                TweenMax.set('.dummy', {
                    pointerEvents: 'auto'
                });
        ...
        $('.scroll-down').on('click', function(e){
            TweenMax.to('.wrapper', 5, {scrollTo: $('.dummy').height()});
            ga('send', 'event', 'intro', 'clicked_scrolldown');
        });
        // Send your own
        $('#sendYourOwnButton').on('click', function(e) {
            App.navigation.to.sendWhat();
            ga('send', 'event', 'intro', 'clicked_sendamessage');
            e.preventDefault();
        });

        // Select option
        $('.nl-field ul li').on('touchstart click', function() {
            ga('send', 'event', 'customize','triedgesture',$('#nl-form select').val());

            if(App.isSent === false){
                App.navigation.to.sendTo();
                App.isSent = true;
            }
            // console.log(window.location.origin + $(this).data('background'));
            App.helpers.setBackgroundPicture('/' + $(this).data('background'));
        });
        // Input field
        $('.name-input').on('input', function() {
            if($(this).val() === '') {
                TweenLite.to($('#createLinkButton'), 0.1, {autoAlpha: 0});
            } else {
                TweenLite.to($('#createLinkButton'), 0.1, {autoAlpha: 1});
            }
        });


        // Copy to clipboard
        var client = new ZeroClipboard($("#urlToSend"));
        client.on('ready', function(event) {
            client.on('copy', function(event){
                ga('send', 'event', 'sendfriend', 'copied_clipboard',$("#urlToSend").val());
                var text = 'http://' + $("#urlToSend").val().toLowerCase();
                var clipboard = event.clipboardData.setData("text/plain", text);
                setTimeout(function(){$("#urlToSend").select();},5);
                TweenMax.to('#urlToSend', 0.15, {
                    scale: 1.05,
                    yoyo: true,
                    ease: Expo.easeInOut,
                    repeat: 1
                });
                var helpermessage = $('.helpermessage').first();
                helpermessage.text('Text copied to the clipboard!');
                TweenMax.to(helpermessage, 0.25, { opacity: 1 });
                TweenMax.to(helpermessage, 0.25, { delay: 1, opacity: 0.4 });
            });
        });
        // Create link button
        $('#createLinkButton').on('click', function(e) {
            App.navigation.to.createLink();
            e.preventDefault();
        });
